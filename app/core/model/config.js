...
export class Config {
  ...
  constructor(aurelia: Aurelia) {
    ...
    this.postTask(() => this.loadResources(aurelia.container, this.resourcesToLoad, aurelia.resources));
  }

  loadResources(container, resourcesToLoad, appResources) {
  ...
  */
  ...
  globalResources(resources: string|string[]): FrameworkConfiguration {
    ...
    for (let i = 0, ii = toAdd.length; i < ii; ++i) {
      ...
      path = Path.join(resourcesRelativeTo, resource);
 ...
 */
  ...
  */
  apply(): Promise<void> {
    ...
    return this.runTasks(this, this.preTasks).then(() => {
      ...
      let next = () => {
        if (current = info.shift()) {
          return this.loadPlugin(this, loader, current).then(next);
        }
      ...
      return next().then(() => this.runTasks(this, this.postTasks));
    });
  }

  loadPlugin(config, loader, info) {
	  this.logger.debug(`Loading plugin ${info.moduleId}.`);
	  ...
	  return loader.loadModule(info.moduleId).then(m => {
	    if ('configure' in m) {
	      return Promise.resolve(m.configure(config, info.config || {})).then(() => {
	        ...
	        this.logger.debug(`Configured plugin ${info.moduleId}.`);
	      });
	    }

	    config.resourcesRelativeTo = null;
	    this.logger.debug(`Loaded plugin ${info.moduleId}.`);
	  });
  }
